...
func TestNewListSupportedPricingApisCommand(t *testing.T) {
	...
	assert.NotContains(t, cmd.Sample, "aliyun list-supported-pricing-apis")
}
...
func TestNewListSupportedPricingApisCommandRunReportsErrors(t *testing.T) {
	// The Run closure wires LoadProfileWithContext + invoke + print together.
	// Easy to introduce a typo that silently returns nil. Drive it end-to-end
	// against an unresolvable host. The closure may fail at profile load
	// (CI environment has no ~/.aliyun/config.json) or at transport (local
	// dev has a profile) — either way the closure must surface a non-nil
