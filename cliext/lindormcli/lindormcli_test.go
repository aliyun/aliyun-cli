...
func TestRun_NotInstalled_FreshInstallAndExecute(t *testing.T) {
	...
	origGetVersion := getLatestLindormCliVersionFunc
	downloadCount := 0
	...
	t.Cleanup(func() {
		...
		getLatestLindormCliVersionFunc = origGetVersion
	})
	...
	if err := c.Run([]string{"lindorm", "help"}); err != nil {
		t.Fatalf("Run failed: %v", err)
	}
	if downloadCount != 1 {
		t.Fatalf("expected download once, got %d", downloadCount)
	}
	data, err := os.ReadFile(filepath.Join(config.GetConfigPath(), ".lindormcli_version_check"))
	if err != nil {
...
func TestRun_Installed_NoVersionCheckWithinTTL(t *testing.T) {
	...
	execPath := filepath.Join(config.GetConfigPath(), "lindorm-open-api-cli")
	if runtime.GOOS == "windows" {
		execPath += ".exe"
	}
	writeExecutable(t, execPath, "dummy")

	_ = os.WriteFile(filepath.Join(config.GetConfigPath(), ".lindormcli_version_check"), []byte(fmt.Sprintf("%d", time.Now().Unix())), 0644)
...
func TestRun_Installed_UpdateWhenExpired(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(config.GetConfigPath(), ".lindormcli_version_check"), []byte(fmt.Sprintf("%d", old)), 0644)
	_ = os.WriteFile(filepath.Join(config.GetConfigPath(), ".lindormcli_version"), []byte("v0.0.0"), 0644)
...
func TestNeedCheckVersionVariants(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(config.GetConfigPath(), ".lindormcli_version_check"), []byte("abc"), 0644)
	c.InitBasicInfo()
	if !c.NeedCheckVersion() {
	...
	_ = os.WriteFile(filepath.Join(config.GetConfigPath(), ".lindormcli_version_check"), []byte(fmt.Sprintf("%d", time.Now().Unix()-int64(VersionCheckTTL)-5)), 0644)
	c.InitBasicInfo()
	if !c.NeedCheckVersion() {
...
func TestInstallUrlAndInvocation(t *testing.T) {
	...
	c.binaryName = "lindorm-open-api-cli_linux_amd64"

	called := false
	origDownload := downloadBinaryFunc
	downloadBinaryFunc = func(url, dest string) error {
		called = true
		expectedURL := ossDownloadBase + c.versionRemote + "/" + "lindorm-open-api-cli_linux_amd64"
		if url != expectedURL {
...
func TestCheckOsTypeAndArchVariants(t *testing.T) {
	...
	tests := []struct {
		...
		{"linux", "amd64", true, "lindorm-open-api-cli_linux_amd64"},
		{"linux", "arm64", true, "lindorm-open-api-cli_linux_arm64"},
		{"darwin", "amd64", true, "lindorm-open-api-cli_darwin_amd64"},
		{"darwin", "arm64", true, "lindorm-open-api-cli_darwin_arm64"},
		{"windows", "amd64", true, "lindorm-open-api-cli_windows_amd64.exe"},
		{"linux", "s390x", false, ""},
		{"unknown", "amd64", false, ""},
	}
...
// --- GetLatestLindormCliVersion ---

func TestGetLatestLindormCliVersion_Success(t *testing.T) {
	...
	ver, err := GetLatestLindormCliVersion()
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if ver != "v1.2.3" {
...
func TestGetLatestLindormCliVersion_NetworkError(t *testing.T) {
	origHTTPDo := httpDoFunc
	defer func() { httpDoFunc = origHTTPDo }()
	httpDoFunc = func(req *http.Request) (*http.Response, error) {
...
func TestGetLatestLindormCliVersion_Non200(t *testing.T) {
	origHTTPDo := httpDoFunc
	defer func() { httpDoFunc = origHTTPDo }()
	httpDoFunc = func(req *http.Request) (*http.Response, error) {
...
func TestGetLatestLindormCliVersion_Empty(t *testing.T) {
	origHTTPDo := httpDoFunc
	defer func() { httpDoFunc = origHTTPDo }()
	httpDoFunc = func(req *http.Request) (*http.Response, error) {
		return &http.Response{
