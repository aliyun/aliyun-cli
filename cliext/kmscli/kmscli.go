...
var (
	...
	getLatestKmscliVersionFunc = GetLatestKmscliVersion
...
const (
	ossDownloadBase = "https://alibaba-kmscli-pub.oss-cn-hangzhou.aliyuncs.com/kmscli/"
)
...
func GetLatestKmscliVersion() (string, error) {
	url := ossDownloadBase + "latest/version.txt"
...
func (c *Context) EnsureInstalledAndUpdated() error {
	if !c.installed {
		latestVersion, err := getLatestKmscliVersionFunc()
		if err != nil {
			return fmt.Errorf("kmscli is not installed and auto-download failed: %v", err)
		}
...
func (c *Context) Install() error {
	url := ossDownloadBase + c.binaryName

	tmpFile := c.execFilePath + ".tmp"
	if err := downloadBinaryFunc(url, tmpFile); err != nil {
		_ = os.Remove(tmpFile)
		return fmt.Errorf("failed to download kmscli from %s: %v", url, err)
	}
	...
	if err := os.Rename(tmpFile, c.execFilePath); err != nil {
		if copyErr := util.CopyFileAndRemoveSource(tmpFile, c.execFilePath); copyErr != nil {
			return fmt.Errorf("failed to install kmscli binary: %v", copyErr)
		}
	}
