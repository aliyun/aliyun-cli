...
func (c *Context) Run(args []string) error {
	...
	return c.ExecuteEsacli(newArgs)
}
...
func (c *Context) InitBasicInfo() error {
	...
	//   unix:    <prefix>/bin/esa-cli
	//   windows: <prefix>/esa-cli.cmd
	if c.osType == "windows" {
		c.execFilePath = filepath.Join(c.prefixPath, "esa-cli.cmd")
	} else {
		c.execFilePath = filepath.Join(c.prefixPath, "bin", "esa-cli")
	}

	if envPath := strings.TrimSpace(os.Getenv("ALIBABA_CLOUD_ESA_CLI_EXEC_PATH")); envPath != "" {
...
// EnsureNodeAvailable detects system node >= minNodeMajor. esa-cli ships
// as ESM with top-level await, so anything older fails with cryptic
// SyntaxError — we surface an actionable hint instead.
func (c *Context) EnsureNodeAvailable() error {
	if envPath := strings.TrimSpace(os.Getenv("ALIBABA_CLOUD_ESA_CLI_NODE_PATH")); envPath != "" {
		c.nodePath = envPath
		return nil
	}
	path, err := lookPathFunc("node")
	if err != nil {
		return fmt.Errorf("node >= %d is required for `aliyun esa-cli`.\n"+
			"  macOS:   brew install node@%d\n"+
			"  Linux:   https://nodejs.org/en/download (LTS)\n"+
			"  Windows: https://nodejs.org/en/download\n"+
			"Or set ALIBABA_CLOUD_ESA_CLI_NODE_PATH=/path/to/node",
...
func (c *Context) EnsureNpmAvailable() error {
	if envPath := strings.TrimSpace(os.Getenv("ALIBABA_CLOUD_ESA_CLI_NPM_PATH")); envPath != "" {
		c.npmPath = envPath
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("npm not found in PATH (looked next to %s and via $PATH).\n"+
			...
			"Or set ALIBABA_CLOUD_ESA_CLI_NPM_PATH=/path/to/npm",
			c.nodePath)
	}
	c.npmPath = path
	return nil
...
func (c *Context) EnsurePrefixAndPackage() error {
	...
	if !c.installed {
		fmt.Fprintln(c.originCtx.Stderr(), "Installing esa-cli...")
		if err := c.npmInstall(false); err != nil {
			return err
		}
		c.installed = fileExists(c.execFilePath)
		if !c.installed {
			return fmt.Errorf("esa-cli install completed but binary not found at %s", c.execFilePath)
		}
		_ = c.UpdateCheckCacheTime()
		return nil
	}

	if os.Getenv("ALIBABA_CLOUD_ESA_CLI_NO_UPDATE_CHECK") == "1" {
		return nil
	}
...
// We do not depend on OSS being authoritative — version.txt is just a
// pinning channel; absence of it falls back to npm registry's latest.
func (c *Context) npmInstall(upgrade bool) error {
	target := npmPackageName
	if v := strings.TrimSpace(os.Getenv("ALIBABA_CLOUD_ESA_CLI_VERSION")); v != "" {
	...
	// -g + --prefix gives global install layout: <prefix>/bin/esa-cli (unix)
	// or <prefix>/esa-cli.cmd (windows). Without -g, npm creates a local
	...
	if registry := strings.TrimSpace(os.Getenv("ALIBABA_CLOUD_ESA_CLI_NPM_REGISTRY")); registry != "" {
	...
	if err := cmd.Run(); err != nil {
		...
		return fmt.Errorf("failed to install %s into %s via npm: %v\n"+
			"Hints:\n"+
			"  - Use a custom registry: export ALIBABA_CLOUD_ESA_CLI_NPM_REGISTRY=...\n"+
			"  - Pin a version:         export ALIBABA_CLOUD_ESA_CLI_VERSION=x.y.z\n"+
			"  - Use a local binary:    export ALIBABA_CLOUD_ESA_CLI_EXEC_PATH=$(which esa-cli)",
...
func (c *Context) effectiveBaseURL() string {
	if u := strings.TrimSpace(os.Getenv("ALIBABA_CLOUD_ESA_CLI_DOWNLOAD_BASE_URL")); u != "" {
		return u
	}
	return downloadBaseURL
}

// applyMainCliFlagsFromArgs mirrors computenestutil: KeepArgs skips flag
...
// PrepareEnv builds the env for the esa-cli subprocess. Mirrors the
// computenestutil approach: profile-derived ALIBABA_CLOUD_* creds (incl.
// RamRoleArn → STS exchange), region, ai-mode UA injection, and a
// COMPAT_MODE marker so esa-cli can adapt help/telemetry.
...
// ExecuteEsacli invokes the resolved esa-cli binary. For v1 we use the
// same stdio wiring as computenestutil; TTY/raw-mode handling for the
// `dev` command will land in a follow-up commit (ink/React requires the
