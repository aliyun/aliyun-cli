...
// String returns a stable lowercase spelling of the Kind.
// Callers serialising Provenance should use this (never the raw int) to keep on-disk provenance.json readable and forward-compatible.
...
type Source interface {
	...
	// so the Loader can distinguish "not here" from "hard failure".
	LoadAPI(code, version, name string) (*meta.API, error)
}

// ErrNotFound is the sentinel returned when a lookup misses.
// Callers use errors.Is to differentiate;
// the Loader depends on this to know when to try the next Source for product ownership.
