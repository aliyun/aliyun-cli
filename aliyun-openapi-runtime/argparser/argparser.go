...
// (no format conversion). Known values are resolved recursively with the
// declared metadata type, matching the legacy runtime's Arg.Resolve behavior
// for JSON object/array/map input as well as key=value input.
