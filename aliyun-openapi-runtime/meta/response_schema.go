...
// ResponseSchemaDocument is the renderer-independent projection of one API's
// selected successful response body. Schema and Components retain JSON
// encoding so callers do not lose OpenAPI fields such as $ref.
...
// every declared response, while Components contains only reachable schemas.
// References are preserved and never recursively inlined.
...
// ResponseSchemaError identifies malformed lazy response metadata.
...
func (e *ResponseSchemaError) Error() string {
	return fmt.Sprintf("parse runtime response %s failed: %v", e.Field, e.Err)
}
...
func (a *API) ResponseSchema() (ResponseSchemaDocument, error) {
	...
	refs, refWarnings, err := schemaReferences(schema)
	if err != nil {
		return ResponseSchemaDocument{}, responseSchemaError("responses", err)
	}
	components, warnings, err := a.reachableComponents(refs, refWarnings)
	if err != nil {
		return ResponseSchemaDocument{}, err
	}
...
// It is separate from ResponseSchema, which selects one success body.
func (a *API) ResponseSection() (ResponseSectionDocument, error) {
	...
	components, warnings, err := a.reachableComponents(refs, refWarnings)
	if err != nil {
		return ResponseSectionDocument{}, err
	}
	document.Components = components
	document.Warnings = warnings
	return document, nil
...
func (a *API) reachableComponents(rootRefs, initialWarnings []string) (map[string]json.RawMessage, []string, error) {
	warnings := newWarningCollector()
	warnings.addAll(initialWarnings)
	if len(rootRefs) == 0 {
		return nil, warnings.values(), nil
	}

	schemas, err := decodeComponentSchemas(a.Components)
	if err != nil {
		return nil, nil, responseSchemaError("components", err)
	}
	...
	visit = func(name string) error {
		...
		dependencies, dependencyWarnings, err := schemaReferences(schema)
		if err != nil {
			return err
		}
	...
	return reachable, warnings.values(), nil
}
