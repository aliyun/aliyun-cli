...
build: metas
	go build -ldflags "-X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun aliyun/main.go

install: build
	cp out/aliyun /usr/local/bin

build_mac:
	GOOS=darwin GOARCH=amd64 go build -o out/aliyun aliyun/main.go
...
build_linux:
	GOOS=linux GOARCH=amd64 go build -o out/aliyun aliyun/main.go
...
build_windows:
	GOOS=windows GOARCH=amd64 go build -o aliyun.exe aliyun/main.go
