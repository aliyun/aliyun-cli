...
安全策略默认关闭，`confirm` 只是 CLI 本地执行前确认，不代表云端授权或审批。当前规则按 CLI 命令模式匹配，尚未启用 API 级别的别名映射；PascalCase 和 kebab-case 入口需要分别配置。使用 `show`/`list` 查看当前生效策略；如果设置了 `ALIBABA_CLOUD_SAFETY_POLICY_ENABLED` 或 `ALIBABA_CLOUD_SAFETY_POLICY_RULES`，展示结果包含环境变量覆盖后的配置。策略文件或环境规则无法完整解析时，CLI 会拒绝继续执行。
